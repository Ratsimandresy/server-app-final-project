...
router.patch("/fav-event", async (req, res, next) => {
    ...
    try {
        ...
        const objectUser = uploadUser.toObject({ getters: true});

        //console.log(objectUser);
        objectUser.events = events;

        console.log(objectUser);
        
        const favEventsUser = await User.findByIdAndUpdate(userId, objectUser, {new: true});
        ...
        res.status(200).json(favEventsUser);
    } catch (errDb) {
        console.log(errDb);
