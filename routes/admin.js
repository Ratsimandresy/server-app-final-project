...
router.post("/users", uploadCloud.single("profilImage"), async (req, res, next) => {
...
router.patch("/categories/:id", async (req, res, next) => {
    try {
        ...
        const updatedCategorie = await Category.findByIdAndUpdate(req.params.id, req.body, {new: true});
...
router.post("/events", uploadCloud.single("mainImageUrl"), async (req, res, next) => {
    ...
    try {
        ...
        res.status(200).json({createdEvent, message: "Event created succefully"});
    } catch (errDb) {
        res.status(500).json({errDb, message: "error can't create a new event"});
    }
});

router.patch("/events/:id", uploadCloud.single("mainImageUrl"), async (req, res, next) => {
    ...
    try {
        const updatedEvent = await Event.findByIdAndUpdate(eventId, even, {new: true});
        console.log(updatedEvent)
        res.status(200).json({updatedEvent, message: "Events succefully updated"});
    } catch (errDb) {
...
router.delete("/events/:id", async (req, res, next) => {
    ...
    try {
        ...
        res.status(200).json({deletedEvent, message: "Events succefully deleted"});
    } catch (errDb) {
